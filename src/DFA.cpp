...
void DFA::add_trans(state_t from, state_t to, char_t ch) {
	...
	trans_[ch][from]=to;
	std::cout << "addDFATrans " << from << " -> " << to << ", ch = " << ch << std::endl;
}
...
std::map<std::size_t, std::size_t> bel;
...
void color(state_t x,int col) {
	...
	bel[x] = col;
	for (std::size_t i = ls[x]; i; i = e[i].next) {
		color(e[i].y, col);
	}
}
...
void DFA::print_to_file(const std::string &filename) const {
	...
	for (std::size_t state = 1; state <= size_; ++ state) {
		for (char_t ch = 0; ch < 256; ++ ch) {
			...
			if (map_result != trans_[ch].end()) {
				...
				fs << "\t" << state << " -> " << tar_state << "[style=bold,label=\" " << ch << "\"];" << std::endl;
	...
	for (std::size_t state=2;state<=size_;++state) {
		if (recv_.find(state) != recv_.end()) {
			fs << "\t" << state << ":body[shape=doublecircle, width=0.4];" << std::endl;
		} else {
...
bool DFA::is_recv() const {
	return recv_.find(state_) != recv_.end();
}