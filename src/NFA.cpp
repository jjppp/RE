...
NFA::NFA(std::size_t size) : size_(size) {}
...
static void printGraphvizList(std::fstream &fs, state_t from, char_t ch,
                              const std::set<state_t> &states) {
  for (state_t to : states) {
    if (ch) {
      fs << "\t" << from << " -> " << to << " [style=bold,label=\" " << ch
         << "\"];" << std::endl;
    } else {
      fs << "\t" << from << " -> " << to << " [style=bold,label=\" ε\"];"
         << std::endl;
    }
  }
}
...
void NFA::print() const {
  ...
  fs << "\t" << size_ - 1 << ":body[shape=doublecircle, width=0.4];"
     << std::endl;
  fs << "}" << std::endl;
}
